...
class KafkaProducer {
   ...
   * kafa 发送message
   */
  def kafkaSend(topic: String, msg: String): Unit =
    try { getProducer.send(new KeyedMessage[String, String](topic, msg.hashCode.toString, msg)) } catch {
  ...
  def kafkaSendList(msgList: List[(String, String)]): Unit = {
    ...
    try {
      producer.send(messageList: _*)
    } catch {
      case e: Exception => e.printStackTrace()
    }
  }

  def getProducer: Producer[String, String] = producermap.getOrElse(brokers, {
